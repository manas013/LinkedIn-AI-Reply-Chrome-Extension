...
const Modal=({isVisible,onClose})=> {
  ...
  const handleInsert = () => {
    ...
    if (msgForm) {
      ...
      newElment.innerText="Thank you for the opportunity! If you have any more questions or if there's anything else I can help you with, feel free to ask."
  ...
  return (
    <div onClick={handleClose}  id='wrapper' className='fixed inset-0  flex justify-center items-center w-635'>
      ...
      <div  key={index} >
          ...
          <footer className="self-stretch rounded-xl shadow-[0px_2px_4px_rgba(0,_0,_0,_0.06)_inset] flex flex-row items-center justify-center py-0 px-1.5 box-border gap-[0px_4px] max-w-full text-left text-5xl text-colors-gray-300 font-new-style-text-2xl-semi-bold">
            ...
            <div className="flex-1 rounded-lg bg-white  overflow-hidden flex flex-row items-center justify-center pt-[5px] px-1 pb-1 max-w-full ">
              <input   value={command} onChange={handleInputChange} className="placeholder:italic h-[52px] min-w-[635px] w-[786px] relative font-medium flex items-center shrink-0 max-w-full mq450:text-lgi placeholder:text-slate-400 block bg-white w-full border border-slate-300 rounded-md py-2 pl-9 pr-3 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 sm:text-sm" placeholder="Your Promt" type="text" name="search"/>
            </div>
          </footer>
          <div className="self-stretch flex flex-row items-start justify-end py-0 px-1.5 m-0 box-border max-w-full">
            ...
            <button onClick={handleInsert}  className="cursor-pointer [border:none]  px-spacing-spacing-24 overflow-hidden flex flex-row items-start justify-start gap-[0px_9px]">
              <div className="rounded-lg overflow-hidden flex flex-row items-center justify-start py-spacing-spacing-12 pr-[23px] pl-spacing-spacing-24 gap-[0px_10px] border-[2px] border-solid border-colors-gray-500">
                <img
                  className="h-[16.8px] w-[14.9px] relative"
                  alt=""
                  src={Insert}
                />
                <div className="relative font-semibold mq450:text-lgi">Insert</div>
              </div>
              </button>:""
    }
              {/* <button disabled={command==""} onClick={handleGenerate} className="cursor-pointer [border:none] py-spacing-spacing-12 px-spacing-spacing-24 bg-blue-500 rounded-lg overflow-hidden flex flex-row items-start justify-start gap-[0px_9px] hover:bg-cornflowerblue">
                {
                  arr.length==0?<img className="h-7 w-7 mt-5 relative" alt="" src={Vector} />:<img className="h-7 w-7 mt-5 relative" alt="" src={RoundIcon} />
    
                }
                <div className="relative text-5xl font-semibold font-new-style-text-2xl-semi-bold text-white text-left mq450:text-lgi">
                {arr.length==0?"Generate":"Regenarate"}
                </div>
              </button> */}
              {
                arr.length==0?
                <button disabled={command==""} onClick={handleGenerate} className="cursor-pointer [border:none] py-spacing-spacing-12 px-spacing-spacing-24 bg-blue-500 rounded-lg overflow-hidden flex flex-row items-start justify-start gap-[0px_9px] hover:bg-cornflowerblue">
                  ...
                  <img className="h-7 w-7 mt-5 relative" alt="" src={Vector} />
                ...
                Generate
                 ...
                 <button  className="cursor-pointer [border:none] py-spacing-spacing-12 px-spacing-spacing-24 bg-blue-500 rounded-lg overflow-hidden flex flex-row items-start justify-start gap-[0px_9px] hover:bg-cornflowerblue">
                 
                  <img className="h-7 w-7 mt-5 relative" alt="" src={RoundIcon} />
